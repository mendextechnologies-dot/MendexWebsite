    ...
    <div className="min-h-screen bg-background text-foreground">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {productsList.map((prod, idx) => (
              <div key={idx} className="bg-card p-6 rounded-2xl border border-border flex items-center gap-3 shadow-sm">
                ...
                <span className="font-semibold text-slate-700 text-sm">{prod}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 px-4 bg-secondary/20 border-t border-border text-center">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl sm:text-4xl font-extrabold mb-6 tracking-tight text-slate-900">Need a Custom Power System?</h2>
